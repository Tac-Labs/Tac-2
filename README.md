...
- All your project code into a single main package (`Tac-2/`)
...
```bash
<root directory>
├── Tac-2/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `Tac-2`
...
2. Clone the `Tac-2` repository

3. Open a shell into your local `Tac-2` directory and run
    ```bash
    ./rename-template.sh Tac-2
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `Tac-2`, in this example)
...
13. Put your main (resp. test) code in `Tac-2/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `Tac-2` package:
```bash
poetry run python -m Tac-2
```
